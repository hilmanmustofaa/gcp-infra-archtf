resource "google_compute_region_backend_service" "compute_region_backend_services" {
  ...
  dynamic "failover_policy" {
    for_each = each.value.failover_policy != null && each.value.load_balancing_scheme == "INTERNAL" && (each.value.protocol == "TCP" || each.value.protocol == "UDP") ? { "failover_policy" = each.value.failover_policy } : {}
  ...
  dynamic "log_config" {
    for_each = each.value.load_balancing_scheme == "INTERNAL_MANAGED" && (each.value.protocol == "HTTP" || each.value.protocol == "HTTPS" || each.value.protocol == "HTTP2") && try(each.value.log_config.enable, false) ? { "log_config" = each.value.log_config } : {}
