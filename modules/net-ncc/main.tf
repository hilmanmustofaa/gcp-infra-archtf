...
resource "google_network_connectivity_hub" "hub" {
  name        = var.resource_prefix != null ? join(var.join_separator, [var.resource_prefix, var.ncc_hub_name]) : var.ncc_hub_name
...
resource "google_network_connectivity_spoke" "vpc_spoke" {
  ...
  name        = var.resource_prefix != null ? join(var.join_separator, [var.resource_prefix, each.key]) : each.key
  location    = "global"
...
resource "google_network_connectivity_spoke" "producer_vpc_network_spoke" {
  ...
  project     = var.project_id
  name        = var.resource_prefix != null ? join(var.join_separator, [var.resource_prefix, "${each.key}-linked-spoke"]) : "${each.key}-linked-spoke"
  location    = "global"
  description = each.value.description
  hub         = google_network_connectivity_hub.hub.id
  labels      = merge(var.spoke_labels, each.value.labels)
